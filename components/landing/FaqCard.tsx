...
export default function FaqCard({ que, ans, isOpen, onClick }: FaqProps) {
    ...
    return (
        <div className="md:w-2xl w-xs rounded-2xl flex flex-col justify-center items-start md:px-7 px-3 md:py-5 py-4 gap-2 bg-[#51149C]">
            <div
            ...
            <div className={`${isOpen ? "block" : "hidden"} flex overflow-hidden transition-all duration-300 justify-start items-start text-left`}>
                <p className="text-gray-300 lg:text-lg md:text-base sm:text-sm text-xs text-left">{ans}</p>
            </div>
        </div>
    )
};
